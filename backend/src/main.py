...
async def add_recipe_to_db(slug: str, session: AsyncSession = Depends(get_session)):
    ...
    try:
        ...
        for ingredient_data in ingredient_list_data:
            ...
            ingredient_amount = (
                ingredient_data["label"].replace(ingredient_name, "").strip()
            )

            if ingredient_amount == "0":
                continue

            # TODO:need to filter ingredient lists.

            # if ingredient repeated, need to decide which to keep. so far between 1 and 32g need to keep 32g (so maybe use g?)
            ...
            if existing_ingredient:
                ...
                for image_data in image_list_data:
                    ...
                    image_obj = ImageURL(
                        url=image_url, width=image_width, ingredient=ingredient_obj
                    )
        ...
        for instruction_step_data in instruction_step_list_data:
            ...
            instruction_step_obj = InstructionStep(
                text=instruction_step_text, order=instruction_step_order
            )
        ...
        basic_ingredients = [
            ingredient["title"] for ingredient in basic_ingredients_data
        ]
        ...
        for recipe_image_data in recipe_image_list_data:
            ...
            image_obj = ImageURL(url=image_url, width=image_width, recipe=recipe_obj)
            session.add(image_obj)
